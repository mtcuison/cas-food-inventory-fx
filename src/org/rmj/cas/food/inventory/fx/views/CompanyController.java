...
public class CompanyController implements Initializable {
    ...
    private void cmdButton_Click(ActionEvent event) {
        ...
        switch (lsButton){
            case "btnBrowse":
                JSONObject lsResult = qsParameter.getCompany(poGRider, "", 1);
                
                if (lsResult != null){
                    openRecord((String) lsResult.get("sCompnyID"));
                }
                break;
    ...
    final ChangeListener<? super Boolean> txtField_Focus = (o,ov,nv)->{
        ...
        if(!nv){ /*Lost Focus*/
            switch (lnIndex){
                ...
                case 3:
                    if (lsValue.length() > 7){
                        ShowMessageFX.Warning(null, pxeModuleName, "Max length for 'Company Code' exceeds the maximum limit.");
                        txtField03.requestFocus();
                        return;
                    } break;
                case 4:
                    if (lsValue.length() > 15){
                        ShowMessageFX.Warning(null, pxeModuleName, "Max length for 'Employer No.' exceeds the maximum limit.");
