...
public class InventoryTypeController implements Initializable {
    ...
    private void cmdButton_Click(ActionEvent event) {
        ...
        switch (lsButton){
            case "btnBrowse":
                JSONObject lsResult = qsParameter.getInvType(poGRider, "", 1);
                
                if (lsResult != null){
                    openRecord((String) lsResult.get("sInvTypCd"));
                }
                break;
            case "btnCancel":
    ...
    final ChangeListener<? super Boolean> txtField_Focus = (o,ov,nv)->{
        ...
        if(!nv){ /*Lost Focus*/
            switch (lnIndex){
                case 1:
                    if (lsValue.length() > 4){
                        ShowMessageFX.Warning(null, pxeModuleName, "Max length for 'Code' exceeds the maximum limit.");
