...
public class ModelController implements Initializable {
    ...
    @FXML private CheckBox Check09;
    @FXML private CheckBox Check08;
    ...
    private void txtField_KeyPressed(KeyEvent event){
        ...
        switch (event.getCode()){
            case F3:
                ...
                switch (lnIndex){
                    case 2:
                        lsResult = qsParameter.getInvType(poGRider, txtField02.getText(), 1);
                        loadMaster(2, lsResult);
                        break;
                    case 6:
                        lsResult = qsParameter.getBrand(poGRider, txtField06.getText(), 1);
                        loadMaster(6, lsResult);
                        break;
                    case 7:
                        lsResult = qsParameter.getCategory2(poGRider, txtField07.getText(), 1);
    ...
    private void cmdButton_Click(ActionEvent event) {
        ...
        switch (lsButton){
            case "btnBrowse":
                JSONObject lsResult = qsParameter.getModel(poGRider, "", 1);
                
                if (lsResult != null){
                    openRecord((String) lsResult.get("sModelCde"));
                }
                break;
    ...
    private void loadRecord(){
        ...
        if (poRecord.getMaster(2) != null && !poRecord.getMaster(2).toString().isEmpty()){
            lsResult = qsParameter.getInvType(poGRider, (String) poRecord.getMaster(2), 0);
            loadMaster(2, lsResult);
        } else
            txtField02.setText("");
        ...
        if (poRecord.getMaster(6) != null && !poRecord.getMaster(6).toString().isEmpty()){
            lsResult = qsParameter.getBrand(poGRider, (String) poRecord.getMaster(6), 0);
            loadMaster(6, lsResult);
        } else
            txtField06.setText("");
        ...
        if (poRecord.getMaster(7) != null && !poRecord.getMaster(7).toString().isEmpty()){
            lsResult = qsParameter.getCategory2(poGRider, (String) poRecord.getMaster(7), 0);
            loadMaster(7, lsResult);
        } else
            txtField07.setText("");
