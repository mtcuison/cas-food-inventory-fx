...
public class SupplierController implements Initializable {
    ...
    @FXML private TextArea txtField11;
    @FXML private CheckBox Check18;
    ...
    private void getMaster(int fnIndex){        
        ...
        switch (fnIndex){
            case 80: /*sClientID*/
                lsResult = qsParameter.searchClientCompany(poGRider, txtField80.getText(), 1); break;
            case 81: /*sBranchCd*/
                lsResult = qsParameter.getBranch(poGRider, txtField81.getText(), 1); break;
            case 12: /*sTermIDxx*/
                lsResult = qsParameter.getTerm(poGRider, txtField12.getText(), 1); break;
            default:
                return;
        }
    ...
    private void cmdButton_Click(ActionEvent event) {
        ...
        switch (lsButton){
            case "btnBrowse":
                JSONObject lsResult = qsParameter.getSupplier(poGRider, "", 1);
                
                if (lsResult != null){
                    openRecord((String) lsResult.get("sClientID"), (String) lsResult.get("sBranchCd"));
                }
                break;
            case "btnCancel":
            ...
            case "btnNew":
                if (poRecord.newRecord()){
                    ...
                    loadMaster(81, qsParameter.getBranch(poGRider, poGRider.getBranchCode(), 0));
    ...
    private void loadRecord(){
        ...
        if (poRecord.getMaster(1) != null && !poRecord.getMaster(1).toString().isEmpty()){
            lsResult = qsParameter.getClientCompany(poGRider, (String) poRecord.getMaster(1), 0);
            loadMaster(80, lsResult);
        }
        
        /*sBranchCD*/
        if (poRecord.getMaster(2) != null && !poRecord.getMaster(2).toString().isEmpty()){
            lsResult = qsParameter.getBranch(poGRider, (String) poRecord.getMaster(2), 0);
            loadMaster(81, lsResult);
        }
        
        /*sTermIDxx*/
        if (poRecord.getMaster(12) != null && !poRecord.getMaster(12).toString().isEmpty()){
            lsResult = qsParameter.getTerm(poGRider, (String) poRecord.getMaster(12), 0);
        ...
        if(!nv){ /*Lost Focus*/
            switch (lnIndex){
                ...
                case 13: /*nDiscount*/
                    ...
                    if (lnDisc > 1){
                        ShowMessageFX.Warning("Discount value must not be grater than 1.", pxeModuleName, "Convert the percentage into decimal form.");
