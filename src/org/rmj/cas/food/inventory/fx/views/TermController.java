...
public class TermController implements Initializable {
    ...
    @FXML private ComboBox cmdField03;
    ...
    private void cmdButton_Click(ActionEvent event) {
        ...
        switch (lsButton){
            case "btnBrowse":
                JSONObject lsResult = qsParameter.getTerm(poGRider, "", 1);
                
                if (lsResult != null){
                    openRecord((String) lsResult.get("sTermCode"));
                }
                break;
    ...
    private boolean sendOtherDetail(){
        if (cmdField03.getSelectionModel().getSelectedIndex() < 0){
            ShowMessageFX.Warning("No coverage selected.", pxeModuleName, "Please select coverage value.");
